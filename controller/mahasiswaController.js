...
export const registerMahasiswa = async (req, res) => {
    try {
        const { nim, nama, jurusan, angkatan, semester, status_kelulusan, password, confirm_password } = req.body;
...
export const loginMahasiswa = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: snapshot.docs[0].id }, process.env.SECRET_KEY, { expiresIn: "1h" });
        res.status(200).send({
            status: "success",
